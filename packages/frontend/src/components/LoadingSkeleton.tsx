export function LoadingSkeleton({ type = 'card' }: { type?: 'card' | 'text' | 'table' | 'page' }) {
  if (type === 'page') {
    return (
      <div className="animate-pulse space-y-6 p-6">
        <div className="h-8 w-48 bg-white/[0.04] rounded-lg" />
        <div className="h-4 w-96 bg-white/[0.04] rounded" />
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[1, 2, 3].map(i => (
            <div key={i} className="h-32 bg-white/[0.03] rounded-2xl border border-white/[0.06]" />
          ))}
        </div>
      </div>
    );
  }

  if (type === 'table') {
    return (
      <div className="animate-pulse space-y-3">
        {[1, 2, 3, 4, 5].map(i => (
          <div key={i} className="h-12 bg-white/[0.03] rounded-lg border border-white/[0.06]" />
        ))}
      </div>
    );
  }

  if (type === 'text') {
    return (
      <div className="animate-pulse space-y-3">
        <div className="h-4 bg-white/[0.04] rounded w-3/4" />
        <div className="h-4 bg-white/[0.04] rounded w-1/2" />
        <div className="h-4 bg-white/[0.04] rounded w-5/6" />
      </div>
    );
  }

  // card
  return (
    <div className="animate-pulse rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6">
      <div className="h-5 bg-white/[0.04] rounded w-2/3 mb-3" />
      <div className="h-4 bg-white/[0.04] rounded w-full mb-2" />
      <div className="h-4 bg-white/[0.04] rounded w-3/4 mb-4" />
      <div className="flex gap-3">
        <div className="h-8 bg-white/[0.04] rounded w-20" />
        <div className="h-8 bg-white/[0.04] rounded w-20" />
        <div className="h-8 bg-white/[0.04] rounded w-20" />
      </div>
    </div>
  );
}
